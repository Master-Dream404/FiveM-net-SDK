...
namespace net
{
	boost::optional<std::string> PeerAddress::LookupServiceRecord(const std::string& serviceHost, uint16_t* servicePort)
	{
		...
		// perform the query
		if (DnsQuery_W(platformHostName.c_str(), DNS_TYPE_SRV, DNS_QUERY_STANDARD, nullptr, &dnsRecords, nullptr) == ERROR_SUCCESS)
		{
			...
			if (dnsRecords && dnsRecords->wType == DNS_TYPE_SRV)
			{
				...
				std::wstring nameString((const wchar_t*)srvData.pNameTarget);
